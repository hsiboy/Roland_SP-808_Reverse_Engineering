...
'      ... <00 00 00> <00 00 00 00 00> <00 00 00> <0C 00 00 00 00> <cksm> F7 
'                     <-- memstart -->            <---memsize --->
'                       (nibble mode)               (nibble mode)

' 10 F0 <len> 41 00 2B 12 <00 00 00> <5-byte mem addr (nibble mode)> <firmware payload> <cksm> F7
...
' sysex event address = 00 00 00, otherwise abort

		If (eventadd( 2 ) <> 0) Or (eventadd( 3 ) <> 0) Then
...
' sysex event address = 01 00 00 -- <00 00 00> <-- memstart --> <00 00 00> <---memsize --->
...
' Specifies memory start address and memory size -- 5-byte (nibble mode)
			...
			If (dwvar1 <> 0) Or (dwvar2 <> 0) Or (memsize = 0) Then
...
Print
Print "Convert Roland VS-880 MIDI firmware files to binary format."
Print
Print "Usage:  RVS82BIN infil=[""input filename""] outfil=[""output filename""]"
Print
Print "Example:  RVS82BIN infil=VS880A-1.mid outfil=VS880A.bin"
Print
Print "To extract firmware from all MIDI files into a single binary file ..."
Print
Print "for %i in (VS880A?.mid) do Rmid2bin.exe infil=%i outfil=VS880A.bin"
